...
class diary extends Contract {
    constructor() {
        super("Dairy products supply chain contract");
    }
     ...
     */
    async registerMilkBatch(ctx, batchId, ranchName, location, amount) {
        ...
        await ctx.stub.putState(
            batchesRegister,
            Buffer.from(JSON.stringify(batches))
        );
        ctx.stub.setEvent(
            "milkBatchRegistered",
            Buffer.from(
    ...
    async transportMilkBatch(ctx, logisticsFirmName, toLocation, batchId) {
        ...
        await ctx.stub.putState(
            batchesTransports,
            Buffer.from(JSON.stringify(transports))
        );
        ctx.stub.setEvent(
    ...
    async registerMilkCarton(
        ...
        await ctx.stub.putState(
            cartonsRegister,
            Buffer.from(JSON.stringify(cartons))
        );
        ctx.stub.setEvent(
            "cartonRegistered",
            Buffer.from(
    ...
    async registerCartonInInventory(ctx, retailerName, cartonId) {
        console.info(
            "============= START : registerCartonInInventory ==========="
        );
    ...
    async registerSelling(ctx, cartonId) {
        ...
        console.log(salesAsBytes.toString());
        const sales = JSON.parse(salesAsBytes.toString());
        sales.push(cartonId);
        await ctx.stub.putState(
            cartonsSales,
            Buffer.from(JSON.stringify(sales))
        );
